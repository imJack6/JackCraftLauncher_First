...
public class MicrosoftLogger
{
    ...
    public static async Task<string[]> GetMicrosoftOAuthToken(string code)
    {
        ...
        var response = await HttpUtils.Get(url, dic);
        var jobj = JsonDocument.Parse(response);
        try
        {
            resultstrs[0] = jobj.RootElement.GetProperty("access_token").GetString() ??
                            throw new Exception("获取MS OAuth Token失败");
            resultstrs[1] = jobj.RootElement.GetProperty("refresh_token").GetString() ??
                            throw new Exception("获取Refresh Token失败");
            resultstrs[2] = Convert.ToString(jobj.RootElement.GetProperty("expires_in").GetInt32()) ??
                            throw new Exception("获取Expires In失败");
            resultstrs[3] = jobj.RootElement.GetProperty("scope").GetString() ?? throw new Exception("获取Scope失败");
            resultstrs[4] = jobj.RootElement.GetProperty("token_type").GetString() ??
                            throw new Exception("获取Token Type失败");
            return resultstrs;
        }
        catch (Exception ex)
        {
